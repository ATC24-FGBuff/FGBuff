...
# LSTM:  [1.0, 0.8486970267957401, 0.8476861647381932, 0.7050942007382809, 0.9526018155706241]
# VGG-16:  [1.0, 0.7252863914698214, 0.5778245071312188, 0.4086413546264609, 0.5975849868716261]
# Resnet-50:  [1.0, 0.7904676105601098, 0.43754368847739483, 0.36365289054633104, 0.7699247388236165]
# BERT-base:  [1.0, 0.6258239798098899, 0.7281553567521688, 0.38112126798709045, 0.6341463418135553]
# FGBuff
...
def test_bar_throughput_related_resnet101():
    ...
    fig.savefig(dst_path + 'resnet101_training_throughput_0102' + ".jpg", dpi=750, bbox_inches='tight') 
    fig.savefig(dst_path + 'resnet101_training_throughput_0102' + ".pdf", dpi=750, bbox_inches='tight')
...
def draw_lstm_ppl_compare_related():
    ...
    time_fg =sum(tensor_np_time_fgbuff_x)
    time_de =sum(tensor_np_time_dear_x)
    time_om =sum(tensor_np_time_omgs_x)
    time_ho =sum(tensor_np_time_horovod_x)
    time_se =sum(tensor_np_time_syncea_x)
    print('LSTM: ', [time_fg/time_fg, time_fg/time_de, time_fg/time_om, time_fg/time_ho, time_fg/time_se])
...
def draw_vgg16_accuracy_compare_related():
    ...
    time_fg =sum(tensor_np_time_fg)
    time_de =sum(tensor_np_time_de)
    time_om =sum(tensor_np_time_om)
    time_ho =sum(tensor_np_time_ho)
    time_se =sum(tensor_np_time_se)
    print('VGG-16: ', [time_fg/time_fg, time_fg/time_de, time_fg/time_om, time_fg/time_ho, time_fg/time_se])
...
def draw_resnet50_accuracy_compare_related():
    ...
    time_ho =sum(tensor_np_time_ho)
    time_se =sum(tensor_np_time_se)
    print('Resnet-50: ', [time_fg/time_fg, time_fg/time_de, time_fg/time_om, time_fg/time_ho, time_fg/time_se])
...
def draw_bar_hatch_bert_accuracy_train_time_compare_related():
    ...
    time_fg = y_train_time[0]
    time_de = y_train_time[1]
    time_om = y_train_time[2]
    time_ho = y_train_time[3]
    time_se = y_train_time[4]
    print('BERT-base: ', [time_fg/time_fg, time_fg/time_de, time_fg/time_om, time_fg/time_ho, time_fg/time_se])
...
def test_bar_hatch_compression_rate_bert():
    ...
    y_sen = [1/1.29, 3.16, 3.73  ] 
    y_hon = [1/2.35, 2.20, 2.53  ]    
    y_omn = [1/1.94, 3.02, 3.59  ]    
    y_den = [1/1.43, 3.22, 3.69  ] 
    y_fgbuff = [1/1.01, 3.91, 4.12 ]
    ...
    y_sen =[y_sen[i]*tp  for i in range(len(y_sen)) ]
    y_hon =[y_hon[i]*tp  for i in range(len(y_hon)) ]
    y_omn =[y_omn[i]*tp  for i in range(len(y_omn)) ]
    y_den =[y_den[i]*tp  for i in range(len(y_den)) ]
    y_fgbuff =[y_fgbuff[i]*tp  for i in range(len(y_fgbuff)) ]
    ...
    # plt.savefig("grace_dll/torch/compressor/global_channel_hybrid_vis_0527/shape_value_bar_channel_missing_bar.pdf", dpi=1400, format='pdf')#eps
    # ax.legend(loc = 2, ncol=1, frameon=False, fontsize=16)
    ax.legend(loc = 9, ncol=3, columnspacing=0.5, labelspacing=0.5, handletextpad=0.2,frameon=False, fontsize=legend_font_size)
...
def test_bar_hatch_bandwidth_bert():
    ...
    # plt.savefig("grace_dll/torch/compressor/global_channel_hybrid_vis_0527/shape_value_bar_channel_missing_bar.pdf", dpi=1400, format='pdf')#eps
    # ax.legend(loc = 2, ncol=1, frameon=False, fontsize=16)
    ax.legend(loc = 2, ncol=1, columnspacing=1.0, labelspacing=0.5, handletextpad=0.2,frameon=False, fontsize=legend_font_size)
...
def test_bar_hatch_different_sparsification_resnet50():
    ...
    labels = ['Gaussiank', 'DGC',  'Redsync', 'Sidco']
    # labels = ['Z', 'U', 'S', 'D', 'X', 'Q', 'D', 'J', 'R', 'R', 'R']
    x = np.arange(len(labels))*1.3
    # labels=['Baseline (Ring-Allreduce)', 'Global Top-k', 'All-Channel Top-k', 'ADTopk']    
    ...
    y_sen = [1/1.29, 3.16, 3.73 , 3.73 ] 
    y_hon = [1/2.35, 2.20, 2.53 , 3.73 ]    
    y_omn = [1/1.94, 3.02, 3.59 , 3.73 ]    
    y_den = [1/1.43, 3.22, 3.69 , 3.73 ] 
    y_fgbuff = [1/1.01, 3.91, 4.12, 3.73 ]
