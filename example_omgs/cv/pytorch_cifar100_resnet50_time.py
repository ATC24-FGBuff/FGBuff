...
def draw_curve(epoch):
    ...
    comp_type = '/cifar100_resnet50_'+str(args.compressor)+'_ef_epoch_'+str(args.epochs)+'_'+str(args.density).replace('.','')
    comp = '/cifar100_resnet50_'+str(args.compressor)+'_ef_epoch_'+str(args.epochs)+'_'+str(args.density).replace('.','')
    
    # resnet50 resnet101 Vgg16,  Vgg19, LSTM ,Bert 
    # 0.1 0.01 0.05
    # topk, gaussiank, readsync dgc , sidoc
    
    
    save_dir = '/home/user/mzq/workspaces/project/dear_pytorch/ATC24-FG-MGS/fgmgs_ours/result_train_omgs'
...
if __name__ == '__main__':
    ...
    # optimizer = hvd.DistributedOptimizer(args.model_net, optimizer, 
    #                                      named_parameters=model.named_parameters(), compression=compressors[args.compressor](), is_sparse=args.density<1, density=args.density, seq_layernames=seq_layernames, layerwise_times=layerwise_times, norm_clip=None, threshold=args.threshold, writer=None)
