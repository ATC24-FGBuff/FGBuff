...
class _DistributedOptimizer(torch.optim.Optimizer):
    def __init__(self, model_net_name, params, model, named_parameters, 
        ...
        # parameter names
        # named_parameters = list(self._model.named_parameters())
    ...
    def _generate_groups_mgwfbp(self):
        ...
        def __merge(taob, tc, p, l):
            ...
            if self.size_commtime_dict is not None:
                ...
                tc[l-1] = utils_bayes.predict_allreduce_time_with_size(alpha, beta, p[l-1]*nbytes, num_of_workers)
        sizes = [self._named_parameters[k].data.numel() for k in self._seq_layernames]
        seq_layernames = self._seq_layernames
        if not utils_bayes.check_unique(seq_layernames):
            raise ValueError
        self._sizes = sizes
        p = sizes[:]
        L = len(sizes)
        if self.size_commtime_dict is not None:
            ...
            tc = [utils_bayes.predict_allreduce_time_with_size(alpha, beta, s*nbytes, num_of_workers) for s in sizes]
    ...
    def _generate_groups_mgs(self):
        ...
        def __calculate_sparse_and_backward_start(tb, sizes, L, start=0):
            taos = [start] * L 
            ts = [utils_bayes.topk_perf_model(s) for s in sizes]
            taob = [start] * L 
            taob[L-1] = start 
        ...
        def __calculate_comm_start(ts, taos, sizes, L):
            taoc = [0] * L 
            tc = [utils_bayes.allgather_perf_model(s, P, self._density) for s in sizes]
        ...
        def __merge(tb, ts, tc, p, l):
            ...
            tc[l-1] = utils_bayes.allgather_perf_model(p[l-1], P, self._density) 
            tc[l] = 0

            ts[l-1] = utils_bayes.topk_perf_model(p[l-1])
            ts[l] = 0
        ...
        for l in range(1, L-1)[::-1]:
            ...
            tw = tb[l-1]+utils_bayes.topk_perf_model(p[l]+p[l-1])\
                - utils_bayes.topk_perf_model(p[l]) - utils_bayes.topk_perf_model(p[l-1])\
                - (taoc[l] - (taos[l]+ts[l]))
            tsave = utils_bayes.allgather_perf_model(p[l], P, self._density)+utils_bayes.allgather_perf_model(p[l-1], P, self._density)-\
                    utils_bayes.allgather_perf_model((p[l]+p[l-1]), P, self._density)
            if tw < tsave:
    ...
    def _generate_merged_parameters(self):
        ...
        for i, g in enumerate(groups):
            ...
            if self._density < 1 and ADAPTIVE_SPARSE:
                _density = utils_bayes.predict_density_with_size_and_computation(sub_size, computation_time, num_of_workers)
    ...
    def _sparse_allreduce_async(self, p, name, density):
        ...
        if self._profiling:
            utils_bayes.force_insert_item(self._compression_timers, name, time.time()-stime)
    ...
    def synchronize(self):
        ...
        for p, value in self._handles.items():
            ...
            if self._sparse and density < 1:
                ...
                if self._profiling:
                    utils_bayes.force_insert_item(self._allreduce_timers, name, time.time()-stime)
                ...
                if self._profiling:
                    utils_bayes.force_insert_item(self._update_times, name, time.time()-stime)
            else:
                stime = time.time()
                output = synchronize(handle)
                if self._profiling:
...
def DistributedOptimizer(model_net_name, optimizer, model=None, named_parameters=None, compression=None, is_sparse=False, density=0.001, seq_layernames=None, layerwise_times=None, norm_clip=None, threshold=0, writer=None, gradient_path=None, momentum_correction=False, fp16=False, mgwfbp=False, rdma=False, asc=False):
    """
    ...
    torch.nn.utils_bayes.clip_grad_norm(model.parameters(), args.clip)
    optimizer.step()
    ```
    ...
    return cls(model_net_name, optimizer.param_groups, model, named_parameters, compression, is_sparse, density, seq_layernames=seq_layernames, layerwise_times=layerwise_times, norm_clip=None, threshold=threshold, writer=writer, gradient_path=gradient_path, momentum_correction=momentum_correction, fp16=fp16, mgwfbp=mgwfbp,rdma=rdma,asc=asc)
