...
def get_compressor(params):
    ...
    if comp == 'dgc':
        from gradce_lib.compressor.dgc import DgcCompressor
        density = params.get('density', 0.3)
        compressor = DgcCompressor(density)
    elif comp == 'none':
        from gradce_lib.compressor.none import NoneCompressor
        compressor = NoneCompressor()
    elif comp == 'topk':
        from gradce_lib.compressor.topk import TopKCompressor
        density = params.get('density', 0.01)
        compressor = TopKCompressor(density,rank=cur_rank)
    
    elif comp == 'gaussiank':
        from gradce_lib.compressor.gaussiank import GaussiankCompressor
        density = params.get('density', 0.01)
        compressor = GaussiankCompressor(density,rank=cur_rank)
    
    elif comp == 'redsync':
        from gradce_lib.compressor.redsync import RedSyncCompressor
        density = params.get('density', 0.01)
        compressor = RedSyncCompressor(density,rank=cur_rank)
    elif comp == 'redsynctrim':
        from gradce_lib.compressor.redsync import RedSyncTrimCompressor
        density = params.get('density', 0.01)
        compressor = RedSyncTrimCompressor(density,rank=cur_rank)
    
    elif comp == 'sidcoexp':
        from gradce_lib.compressor.sidco import ExpCompressor
        density = params.get('density', 0.01)
        compressor = ExpCompressor(density)
    elif comp == 'sidcogp':
        from gradce_lib.compressor.sidco import GParetoCompressor
        density = params.get('density', 0.01)
        compressor = GParetoCompressor(density)
    elif comp == 'sidcogam':
        from gradce_lib.compressor.sidco import GammaGParetoCompressor
        density = params.get('density', 0.01)
        compressor = GammaGParetoCompressor(density)

    elif comp == 'topkef':
        from gradce_lib.compressor.topkef import TopKEFCompressor
        # density = params.get('density', 0.3)
        ...
        from gradce_lib.compressor.randomk import RandomKCompressor
        # density = params.get('density', 0.3)
        density = params.get('density', 0.01)
        model_named_parameters = params.get('model_named_parameters')
        # density = params.get('density', 0.001)
        ...
        from gradce_lib.compressor.imbalancetopktime import ImbalanceTopkTimeCompressor
        density = params.get('density', 0.01)
        model_named_parameters = params.get('model_named_parameters')
...
def get_memory(params):
    ...
    if mem == 'dgc':
        from gradce_lib.memory.dgc import DgcMemory
        momentum = params.get('momentum', 0.9)
        gradient_clipping = params.get('gradient_clipping', False)
        memory = DgcMemory(momentum, gradient_clipping)
    elif mem == 'none':
        from gradce_lib.memory.none import NoneMemory
        memory = NoneMemory()
   
    elif mem == 'residual':
        from gradce_lib.memory.residual import ResidualMemory
        memory = ResidualMemory()
    elif mem == 'residualgtopk':
        from gradce_lib.memory.residualgtopk import ResidualGlobalTopkMemory
...
def get_communicator(params):
    ...
    if comm == 'allreduce':
        from gradce_lib.communicator.allreduce import Allreduce
        return Allreduce(compressor, memory)
    elif comm == 'allgather':
        from gradce_lib.communicator.allgather import Allgather
